...
class TestBuildExampleSensorDefs:
    ...
    def test_channel_names_match_build_ukf_catalog(self):
        ...
        # getting-started substrate list. The example defs must hit
        # the same names so build_ukf + DataFrameSensorSource line up.
        defs = build_example_sensor_defs()
        names = {d.ukf_channel for d in defs}
        assert {"Q_gas", "Q_ch4", "pH"}.issubset(names)
        assert {"Q_maize_silage", "Q_slurry", "Q_cereal_silage"}.issubset(names)
    ...
    def test_density_override_changes_converter(self):
        ...
        maize_default = next(d for d in defs_default if d.ukf_channel == "Q_maize_silage")
...
class TestExampleDefsWithDataFrameSource:
    ...
    def test_full_pipeline_yields_all_channels(self):
        ...
        _, y0 = rows[0]
        # All 6 UKF channels should be populated.
        assert set(y0) == {
            "Q_gas", "Q_ch4", "pH",
            "Q_maize_silage", "Q_slurry", "Q_cereal_silage",
        }
    ...
    def test_unit_conversions_round_trip(self):
        ...
        assert y0["Q_cereal_silage"] == pytest.approx(2.4)
